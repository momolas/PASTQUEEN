//
//  BallisticManager.swift
//  PASTQUEEN
//
//  Created by Mo on 02/11/2023.
//
...
class BallisticManager {
	...
	let MayewskiConstant: Double = 246
	...
	private(set) var path: Double
	private(set) var elevationMOA: Double
	private(set) var windageMOA: Double
	
	var elevationClick: Double {
		return elevationMOA * 100 / (100 / moaClick)
	}
	var windageClick: Double {
		return elevationMOA * 100 / (100 / moaClick)
	}
	...
	init(distanceOfInterest: Double, muzzleVelocity: Double, bulletWeight: Double, ballisticCoefficient: Double, windSpeed: Double, windDirection: Double, temperature: Double, altitude: Double, pressure: Double, scopeHeight: Double, zeroRange: Double) {
		...
		self.retardCoefficient = ballisticCoefficient * MayewskiConstant * pow(muzzleVelocity, 0.45)
		...
		self.elevationMOA = -path / distanceOfInterest / moaValue * 100
		self.windageMOA = (79.2 * distanceOfInterest * windSpeed / muzzleVelocity / (self.adjretardCoefficient / distanceOfInterest - 1 - self.retardCoefficientRate)) / distanceOfInterest / moaValue * 100 * sin(windDirection / 12 * 2 * Double.pi)
	...
	func calculatePath() {
		...
		self.retardCoefficient = self.ballisticCoefficient * MayewskiConstant * pow(self.muzzleVelocity, 0.45)
		...
		self.elevationMOA = -self.path / self.distanceOfInterest / self.moaValue * 100
		self.windageMOA = (79.2 * self.distanceOfInterest * self.windSpeed / self.muzzleVelocity / (self.adjretardCoefficient / self.distanceOfInterest - 1 - self.retardCoefficientRate)) / self.distanceOfInterest / self.moaValue * 100 * sin(self.windDirection / 12 * 2 * Double.pi)
	...
	*/
	func recalculatePath(distanceOfInterest: Double) {
		self.distanceOfInterest = distanceOfInterest
	
		self.calculatePath()
	}
	
	func recalculatePath(distanceOfInterest: Double, windSpeed: Double, windDirection: Double) {
		self.distanceOfInterest = distanceOfInterest
		self.windSpeed = windSpeed
		self.windDirection = windDirection
	...
	func recalculatePath(temperature: Double, altitude: Double, pressure: Double) {
		self.temperature = temperature
		self.altitude = altitude
		self.pressure = pressure
	...
	func recalculatePath(scopeHeight: Double, zeroRange: Double) {
		self.scopeHeight = scopeHeight
		self.zeroRange = zeroRange
	...
	func recalculatePath(bulletWeight: Double, muzzleVelocity: Double, ballisticCoefficient: Double) {
		self.bulletWeight = bulletWeight
		self.muzzleVelocity = muzzleVelocity
		self.ballisticCoefficient = ballisticCoefficient
	...
	*/
	func recalculatePath(distanceOfInterest: Double, muzzleVelocity: Double, bulletWeight: Double, ballisticCoefficient: Double, windSpeed: Double, windDirection: Double, temperature: Double, altitude: Double, pressure: Double, scopeHeight: Double, zeroRange: Double) {
		self.temperature = temperature
		self.altitude = altitude
		self.pressure = pressure
		self.windSpeed = windSpeed
