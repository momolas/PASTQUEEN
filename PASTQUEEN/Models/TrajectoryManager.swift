...
class TrajectoryManager {
	...
	private(set) var path: Double // Use this to know actual drop in inches at the distance
	...
	init(distanceOfInterest: Double, muzzleVelocity: Double, bulletWeight: Double, ballisticCoefficient: Double, windSpeed: Double, windDirection: Double, temperature: Double, altitude: Double, pressure: Double, scopeHeight: Double, zeroRange: Double) {
		...
		self.windageMOA = (79.2 * distanceOfInterest * windSpeed / muzzleVelocity / (self.adjretardCoefficient / distanceOfInterest - 1 - self.retardCoefficientRate)) / distanceOfInterest / moaValue * 100 * sin(windDirection / 12 * 2 * PI)
	...
	func runCalc() {
		...
		self.windageMOA = (79.2 * self.distanceOfInterest * self.windSpeed / self.muzzleVelocity / (self.adjretardCoefficient / self.distanceOfInterest - 1 - self.retardCoefficientRate)) / self.distanceOfInterest / self.moaValue * 100 * sin(self.windDirection / 12 * 2 * PI)
...
/*
// Example use
let trajectoryManager = TrajectoryManager(distanceOfInterest: 500, muzzleVelocity: 2820, bulletWeight: 120, ballisticCoefficient: 0.297, windSpeed: 10.0, windDirection: 9.0, temperature: 65, altitude: 500, pressure: 1000, scopeHeight: 2.0, zeroRange: 200)

// Calculate the trajectory of the bullet
 trajectoryManager.runCalc()

// Get the elevation MOA
let elevationMOA = ballisticManager.elevationMOA

// Get the windage MOA
let windageMOA = ballisticManager.windageMOA

// Print the results
print("Elevation MOA:", elevationMOA)
print("Windage MOA:", windageMOA)
*/
